...
`Base.or_any` on the Julia versions that define them. For any other operator this throws an
error; packages can add a method for an operator whose neutral element they know.
"""
neutral_element(op, T) =

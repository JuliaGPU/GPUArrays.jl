...
LinearAlgebra.scale!(s::Number, X::GPUArray) = scale!(X, s)
function LinearAlgebra.scale!(X::GPUArray{T}, s::Number) where T <: BLAS.BlasComplex
...
function LinearAlgebra.scale!(X::GPUArray{T}, s::Number) where T <: Union{Float32, Float64}

...
function LinearAlgebra.adjoint!(B::AbstractGPUVector, A::AbstractGPUMatrix)
    ...
    adjoint_kernel!(backend(B))(B, A, ndrange = size(B))
    B
end
function LinearAlgebra.adjoint!(B::AbstractGPUMatrix, A::AbstractGPUVector)
    ...
    adjoint_kernel!(backend(A))(B, A, ndrange = size(A))
    B
end
...
function transpose_f!(f, B::AbstractGPUMatrix{T}, A::AbstractGPUMatrix{T}) where T
    ...
    transpose_kernel!(backend(B))(B, A, ndrange = size(A))
    B
end
...
function LinearAlgebra.copytri!(A::AbstractGPUMatrix{T}, uplo::AbstractChar, conjugate::Bool=false) where T
    ...
    if uplo == 'U' && conjugate
        ...
        U_conj!(backend(_A), ndrange = size(_A))
    elseif uplo == 'U' && !conjugate
        ...
        U_noconj!(backend(_A))(_A, ndrange=size(_A))
    elseif uplo == 'L' && conjugate
        ...
        L_conj!(backend(_A))(_A, ndrange = size(_A))
    elseif uplo == 'L' && !conjugate
        ...
        L_noconj!(backend(_A))(_A, ndrange = size(_A))
    else
...
function LinearAlgebra.tril!(A::AbstractGPUMatrix{T}, d::Integer = 0) where T
  ...
  tril_kernel!(backend(_A))(_A, _d, ndrange = size(_A))
  return A
end
...
function LinearAlgebra.triu!(A::AbstractGPUMatrix{T}, d::Integer = 0) where T
  ...
  triu_kernel!(backend(_A))(_A, _d, ndrange = length(_A))
  return A
end
...
function generic_matmatmul!(C::AbstractArray{R}, A::AbstractArray{T}, B::AbstractArray{S}, a::Number, b::Number) where {T,S,R}
    ...
    matmatmul_kernel!(backend(C))(C, A, B, ndrange = size(C))
    C
end
...
function generic_rmul!(X::AbstractArray, s::Number)
    ...
    rmul_kernel!(backend(X))(X, s, ndrange = size(X))
    return X
end
...
function generic_lmul!(s::Number, X::AbstractArray)
    ...
    lmul_kernel!(backend(X))(X, s, ndrange = size(X))
    return X
end
...
function _permutedims!(::Type{IT}, dest::AbstractGPUArray, src::AbstractGPUArray,
    ...
    permutedims_kernel!(backend(dest))(dest, src, size_src, dest_strides_perm,
                        ndrange = size(dest))
    return dest
end
...
function LinearAlgebra.rotate!(x::AbstractGPUArray, y::AbstractGPUArray, c::Number, s::Number)
    ...
    rotate_kernel!(backend(x))(x, y, c, s, ndrange = size(x))
    return x, y
end
...
function LinearAlgebra.reflect!(x::AbstractGPUArray, y::AbstractGPUArray, c::Number, s::Number)
    ...
    reflect_kernel!(backend(x))(x, y, c, s, ndrange = size(x))
    return x, y
end

...
@inline function _copyto!(dest::AbstractArray, bc::Broadcasted)
    ...
    @kernel function broadcast_kernel_linear(dest, bcstyle, bcf, bcaxes, bcargs...)
    ...
    @kernel function broadcast_kernel_cartesian(dest, bcstyle, bcf, bcaxes, bcargs...)
        bc′ = @static if VERSION >= v"1.10-"
            Broadcasted(bcstyle, bcf, bcargs, bcaxes)
        else
            Broadcasted{bcstyle}(bcf, bcargs, bcaxes)
    ...
    if ndims(dest) == 1 || (isa(IndexStyle(dest), IndexLinear) &&
        ...
        broadcast_kernel_linear(get_backend(dest))(dest, bcstyle,
            bc.f, bc.axes, bc.args...;
            ndrange = length(size(dest)) > 0 ? length(dest) : 1)
    else
        broadcast_kernel_cartesian(get_backend(dest))(dest, bcstyle,
            bc.f, bc.axes, bc.args...;
...
function Base.map!(f, dest::AnyGPUArray, xs::AbstractArray...)
    ...
    heuristic = launch_heuristic(get_backend(dest), map_kernel, dest, bc, 1;
                                 elements, elements_per_thread)
    config = launch_configuration(get_backend(dest), heuristic;
                                  elements, elements_per_thread)
    map_kernel(get_backend(dest))(dest, bc, config.elements_per_thread,
